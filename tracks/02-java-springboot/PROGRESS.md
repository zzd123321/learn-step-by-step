...
- L022：新增一条 `SHIPPED` 订单，并筛选所有非 `CANCELLED` 订单。
- L022：用 Stream 得到所有订单状态列表：`List<String>`。
- L022：用 `count()` 统计金额大于等于 `100.0` 的订单数量。
- L022：把某条 Stream 改写成普通 `for` 循环，对比可读性。
- L023：把金额倒序改成金额升序。
- L023：先筛选 `PAID` 订单，再按金额倒序取前 1 条。
- L023：用 Stream 得到去重后的订单金额列表：`List<Double>`。
- L023：新增同金额订单，并给排序增加第二规则：金额相同按订单 ID 升序。
- L024：新增空订单列表，观察 `anyMatch`、`allMatch`、`noneMatch` 的结果。
- L024：新增 `hasHighValueOrder`，判断是否存在金额大于等于 `200.0` 的订单。
- L024：新增 `allStatusKnown`，判断所有状态是否属于已知状态集合。
- L024：把重复 ID 校验改写成普通 `for` 循环，对比可读性。

## 当前状态

- 已完成 L001、L002、L003、L004、L005、L006、L007、L008、L009、L010、L011、L012、L013、L014、L015、L016、L017、L018、L019、L020、L021、L022、L023、L024 课程正文与最小可运行示例。
...
- L022 示例已使用本机 Java `25.0.3 LTS` 编译并运行验证。
- L023 示例已使用本机 Java `25.0.3 LTS` 编译并运行验证。
- L024 示例已使用本机 Java `25.0.3 LTS` 编译并运行验证。
